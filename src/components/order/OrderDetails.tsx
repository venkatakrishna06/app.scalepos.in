                        ...
                        {order.sub_total > 0 && (
                            <div className="text-xs text-muted-foreground mb-1 space-y-0.5">
                                ...
                                {getGstDetails(order).sgstAmount > 0 && (
                                    <p>SGST ({getGstDetails(order).sgstRate}%):
                                        ₹{getGstDetails(order).sgstAmount.toFixed(2)}</p>
                                )}
                                {getGstDetails(order).cgstAmount > 0 && (
                                    <p>CGST ({getGstDetails(order).cgstRate}%):
                                        ₹{getGstDetails(order).cgstAmount.toFixed(2)}</p>
                                )}
                            </div>
                        )}
                        <p className="text-sm font-medium">Total: <span
